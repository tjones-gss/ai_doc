...
Clinic environments at GSS each have a company code with associated metadata — version, modules enabled, configuration quirks. `clinic-utilities` exposes this info to your AI agent via MCP so you can ask questions like "what version is Clinic XYZ on?" without digging through admin panels.

## Why use it

- **Faster Clinic lookups.** Ask your AI agent instead of clicking around.
- **Context for support work.** When your AI agent is helping triage a customer issue, knowing the Clinic configuration up front saves a step.
