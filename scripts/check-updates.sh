...
#      refreshed article, the issue is labeled `needs-articles` (a hook a Cursor
#      Automation can trigger on).
...
#      metadata + README pulled with GH_PAT. Stage 2 reads these (needs access
#      only to THIS repo) and writes or refreshes the polished catalog articles,
#      then deletes the consumed handoffs.
...
update_and_commit() {
  ...
  [ "${n:-0}" -gt 0 ] 2>/dev/null && msg="Auto: queue $n catalog handoff(s) + state — $DATE_UTC [skip ci]"
...
# ---- Main ----------------------------------------------------------------
ISSUE_BODY="_Generated by \`scripts/check-updates.sh\` on $DATE_UTC. New, changed, and missing-coverage tools are queued in data/new-tools/ for the Stage-2 catalog agent._"
...
ISSUE_BODY+=$'\n\n---\n\n_Stage 1 of 2 — Stage 2 (Cursor Automation) turns data/new-tools/ into new or refreshed catalog articles. The goal is complete LaunchPad / InternalTools coverage for the future GSS Catalog and RAG retrieval layer._'
